...
# merge mastodon data
mastodon_tempo_result = User.search(
    index="mastodon_melbourne_past",
    body={
        "size": 0,
        "aggs": {
            "by_date": {
                "date_histogram": {"field": "creat_time", "calendar_interval": "day"},
                "aggs": {
                    "average_sentiment": {"avg": {"field": "sentiment"}},
                    "max_sentiment": {"max": {"field": "sentiment"}},
                    "min_sentiment": {"min": {"field": "sentiment"}},
                    "contents": {"terms": {"field": "content", "size": 100}},
                    "tags": {"significant_terms": {"field": "tag", "size": 50}},
                },
            }
        },
    },
...
for bucket in mastodon_tempo_result["aggregations"]["by_date"]["buckets"]:
    date = datetime.strptime(bucket["key_as_string"], "%Y-%m-%dT%H:%M:%S.%fZ").strftime(
        "%Y-%m-%d"
    )
...
# merge bom data
bom_tempo_result = User.search(
    index="bom_melbourne_weather_past",
    body={
        "size": 450,
        "_source": [
            "Date",
...
for data in bom_tempo_result["hits"]["hits"]:
    source = data["_source"]
    date = source["Date"]
    aver_temp, min_temp, max_temp = get_tem(
        source["Minimum temperature (C)"], source["Maximum temperature (C)"]
    )
    bom_data[date] = {
        "min_temp": min_temp,
        "max_temp": max_temp,
...
def insert_data(index_name, data):
    actions = [
        {"_index": index_name, "_id": doc["Date"], "_source": doc} for doc in data
    ]
